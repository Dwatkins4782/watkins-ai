...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Show email verification state instead of silent redirect
      setRegistered(true);
        ...
        <div style={{ background: "rgba(28,28,32,0.6)", backdropFilter: "blur(16px)", border: "1px solid var(--border-subtle)", borderRadius: 20, padding: "40px 36px" }}>
          {registered ? (
            <div style={{ textAlign: "center" as const }}>
              ...
              <p style={{ fontSize: 13, color: "var(--text-tertiary)" }}>Can&apos;t find it? Check your spam folder. Link expires in 24 hours.</p>
            </div>
          ) : (
            <>
              <h1 style={{ fontSize: 28, fontWeight: 700, marginBottom: 8, textAlign: "center" }}>Create Account</h1>
              <p style={{ fontSize: 15, color: "var(--text-secondary)", textAlign: "center", marginBottom: 32 }}>Start your free 14-day trial. No credit card required.</p>
              ...
              <p style={{ fontSize: 12, color: "var(--text-tertiary)", textAlign: "center", marginTop: 16, lineHeight: 1.6 }}>
              ...
              <p style={{ textAlign: "center", fontSize: 14, color: "var(--text-tertiary)", marginTop: 16 }}>Already have an account? <a href="/auth/login" style={{ color: "#818cf8", textDecoration: "none", fontWeight: 500 }}>Log in</a></p>
            </>
          )}
        </div>
        {/* FIXED: Removed false "SOC 2 Compliant" claim */}
        <div style={{ display: "flex", justifyContent: "center", gap: 24, marginTop: 24, fontSize: 12, color: "var(--text-tertiary)" }}>
          <span>256-bit SSL</span><span>SOC 2 Type II in progress</span><span>14-day free trial</span>
        </div>
      </div>
    </div>
  );

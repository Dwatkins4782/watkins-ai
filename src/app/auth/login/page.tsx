  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 24px", background: "var(--bg-primary)", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ width: "100%", maxWidth: 440, position: "relative", zIndex: 1 }}>
        ...
        <div style={{ background: "rgba(28,28,32,0.6)", backdropFilter: "blur(16px)", border: "1px solid var(--border-subtle)", borderRadius: 20, padding: "40px 36px" }}>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom: 16 }}>
              ...
              <input type="email" required value={form.email} onChange={update("email")} placeholder="john@example.com" style={inputStyle} onFocus={e => e.target.style.borderColor = "#6366f1"} onBlur={e => e.target.style.borderColor = ""} />
            </div>
            <div style={{ marginBottom: 8 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                ...
                <a href="/auth/forgot-password" style={{ fontSize: 13, color: "var(--accent-primary)", textDecoration: "none" }}>Forgot password?</a>
              </div>
              <input type="password" required value={form.password} onChange={update("password")} placeholder="••••••••" style={inputStyle} onFocus={e => e.target.style.borderColor = "#6366f1"} onBlur={e => e.target.style.borderColor = ""} />
            </div>
        ...
        {/* FIXED: Removed false "SOC 2 Compliant" — replaced with accurate "in progress" */}
        <div style={{ display: "flex", justifyContent: "center", gap: 24, marginTop: 24, fontSize: 12, color: "var(--text-tertiary)" }}>
          <span>256-bit SSL</span><span>SOC 2 Type II in progress</span>
        </div>
      </div>
    </div>
  );
